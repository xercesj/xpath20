...
public class AbstractPsychoPathTest extends XMLTestCase {
	...
	protected void loadDOMDocumentXS11(URL fileURL, URL schemaURL) throws Exception {
		InputStream schemaIs = testResolve(schemaURL);
		Schema jaxpSchema = getSchemaXS11(schemaIs);
		Validator v = jaxpSchema.newValidator();
		PSVIDocumentImpl resultDoc = new PSVIDocumentImpl();
		v.validate(new DOMSource(getDomDocument(fileURL.toString())), new DOMResult(resultDoc));
		domDoc = (Document)resultDoc;
	}
	
	private Document getDomDocument(String docUri) throws Exception {
	...
	private Schema getSchemaXS11(InputStream schemaIs) throws SAXException {
	...
	protected XSModel getGrammarXS11(URL schemaURL) throws IOException, SAXException {
		InputStream schemaIs = testResolve(schemaURL);
		SchemaFactory sf = SchemaFactory.newInstance("http://www.w3.org/2001/XMLSchema/v1.1");
		Schema schema = sf.newSchema(new StreamSource(schemaIs));
		XSGrammarPoolContainer poolContainer = (XSGrammarPoolContainer) schema;
	...
	protected void assertXPathFalse(String xpath, DynamicContext dc, Document domDoc) {
		XSBoolean result = evaluateSimpleXPath(xpath, dc, domDoc, XSBoolean.class);
